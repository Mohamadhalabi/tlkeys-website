...
type AttrFacet = { slug: string; name: string; priority: number; items: FacetItem[] }
...
const filtered = computed(() => {
  const q = (props.searchQ || '').toLowerCase().trim()
  return q
    ? props.attr.items.filter(i =>
    ...
    <div v-show="open" class="px-4 pb-4">
      <div class="relative mb-3">
        ...
        <svg class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M12.9 14.32a7 7 0 1 1 1.414-1.414l3.387 3.387-1.414 1.414-3.387-3.387zM8 13a5 5 0 1 0 0-10 5 5 0 0 0 0 10z" clip-rule="evenodd"/>
        </svg>
      </div>

      <div class="space-y-1.5 max-h-64 overflow-auto pr-1">
        <div v-for="f in filtered" :key="f.slug"
             class="group flex items-center justify-between gap-3 rounded-lg px-2 py-1.5 hover:bg-gray-50">
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox"
                   :checked="selected.includes(f.slug)"
                   @change="emit('pick', f.slug)"
