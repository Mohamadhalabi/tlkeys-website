      ...
      <div class="mx-auto px-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3 ml-auto">
          <NuxtLinkLocale to="/account" class="flex items-center">
            <UserPlusIcon class="w-7 h-7 text-gray-700" />
          </NuxtLinkLocale>

          <!-- Cart icon with badge -->
          <NuxtLinkLocale to="/cart" class="relative flex items-center" aria-label="Cart">
            <ShoppingCartIcon class="w-8 h-8 text-gray-700" />
            <span
              class="absolute -top-1 -right-2 min-w-[1.25rem] h-5 px-1
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1">
              ...
              <select
                ...
                class="w-full border border-gray-300 rounded-lg px-3 py-2 bg-white text-sm focus:outline-none"
              >
...
const { currency, setCurrency, options: currencyOptions, formatMoney } = useCurrency()
...
/* fetchers */
async function fetchCars(){ loadingCars.value = true; errorCars.value=''; try{ const res = await $customApi(`/get_cars_menu`, { method:'GET' }); cars.value = extractCars(res) }catch(err:any){ errorCars.value = err?.data?.message || err?.message || (t('error') as string) }finally{ loadingCars.value=false } }
async function fetchManufacturers(){ loadingManufacturers.value = true; errorManufacturers.value=''; try{ const res = await $customApi(`/get_manufacturers_menu`, { method:'GET' }); manufacturers.value = extractManufacturers(res) }catch(err:any){ errorManufacturers.value = err?.data?.message || err?.message || (t('error') as string) }finally{ loadingManufacturers.value=false } }
async function fetchKeysRemotes(){ loadingKeys.value = true; errorKeys.value=''; try{ const res = await $customApi(`/get_keys_and_remotes_menu`, { method:'GET' }); keysRemotes.value = extractKeysRemotes(res) }catch(err:any){ errorKeys.value = err?.data?.message || err?.message || (t('error') as string) }finally{ loadingKeys.value=false } }
async function fetchDevicesMachines(){ loadingDevices.value = true; errorDevices.value=''; try{ const res = await $customApi(`/get_devices_and_machines`, { method:'GET' }); devicesMachines.value = extractDevicesMachines(res) }catch(err:any){ errorDevices.value = err?.data?.message || err?.message || (t('error') as string) }finally{ loadingDevices.value=false } }
async function fetchAccessoriesTools(){ loadingAccessories.value = true; errorAccessories.value=''; try{ const res = await $customApi(`/get_accessories_and_tools`, { method:'GET' }); accessoriesTools.value = extractAccessoriesTools(res) }catch(err:any){ errorAccessories.value = err?.data?.message || err?.message || (t('error') as string) }finally{ loadingAccessories.value=false } }
async function fetchSoftwareTokens(){ loadingSoftTok.value = true; errorSoftTok.value=''; try{ const res = await $customApi(`/get_softwares_and_tokens`, { method:'GET' }); const {softwares:s,tokens:tks}=extractSoftwareTokens(res); softwares.value=s; tokens.value=tks }catch(err:any){ errorSoftTok.value = err?.data?.message || err?.message || (t('error') as string) }finally{ loadingSoftTok.value=false } }
