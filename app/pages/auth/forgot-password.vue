...
import { ref } from 'vue'
import { useRoute, useNuxtApp, useI18n } from '#imports'
...
async function submit() {
  ...
  try {
    ...
    notice.value = {
      type: 'success',
      text: t('auth.forgot.success', 'Your password reset link has been sent to your email.')
    }
  } catch (e: any) {
    const msg =
      e?.data?.message ||
      e?.message ||
      t('auth.forgot.error', 'Unable to send reset email. Please try again.')
...
function contactWhatsApp() {
  const text = encodeURIComponent(t('auth.forgot.whatsappText', 'I have a problem with resetting my password'))
...
<template>
  <div class="container mx-auto px-4 py-10">
    <div class="max-w-2xl mx-auto">
      <div
        class="rounded-2xl shadow border border-gray-300"
        style="border-top-width: 4px; border-top-color: #ff832d"
      >
        <div class="p-6">
          <form @submit.prevent="submit" class="space-y-5">
            <p class="text-gray-800">
              {{ t('auth.forgot.description', 'Lost your password? Please enter your email address. You will receive a link to create a new password via email.') }}
            </p>
            ...
            <div>
              ...
              <input
                ...
                :placeholder="t('auth.forgot.placeholder', 'you@example.com')"
                autocomplete="email"
              />
            </div>
            ...
            <div class="flex items-center justify-between">
              <NuxtLinkLocale to="/auth/login-register" class="text-sm text-gray-700 hover:underline">
                {{ t('auth.forgot.backToLogin', 'Click here to login') }}
              </NuxtLinkLocale>
              ...
              >
                {{ loading ? t('auth.forgot.sending', 'Sending…') : t('auth.forgot.cta', 'Reset Password') }}
              </button>
            </div>
