...
useHead({
  title: 'VIN → PIN (NEW) | Sandbox Demo',
  meta: [
    { name: 'description', content: 'Demo sandbox for VIN to PIN (NEW). Enter a VIN to retrieve key and pin codes.' },
  ],
})
...
async function submitNew() {
  ...
  try {
    const data = await $customApi('/sandbox/vin-to-pin-new', {
      method: 'POST',
      body: { vin: vin.value, username: username.value },
    })

    if ((data as any)?.error) { errorMessage.value = (data as any).error; return }
...
<template>
  <main class="min-h-screen bg-black text-white flex items-center justify-center p-6">
    <div class="w-full max-w-xl">
      <h1 class="text-center text-2xl font-semibold mb-6">Demo — VIN → PIN (NEW)</h1>
      ...
      <div class="space-y-4">
        ...
        <button class="btn" :disabled="isLoading" @click="submitNew">
